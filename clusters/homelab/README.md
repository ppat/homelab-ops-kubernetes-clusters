...
| `services/ci-diagnostics/` | A `CronJob` that pulls the chainsaw suites' published diagnostics grammar (`READY`/`MODE`/`PULL`/`CONTENTION`/`UNCENSORED`…) from the GitHub Actions API into Loki, so the series outlives the 90 days GitHub keeps job logs, with a Grafana dashboard over it (`services/monitoring/dashboards/ci-diagnostics.json`). Pulls from the homelab deliberately — no inbound path, no CI-side secret, CI stays ignorant it exists. Needs a hand-created Bitwarden entry before it can run; see that directory's `README.md`. Not consumed by any module's manifests and not a security boundary or new namespace, so it goes through the shared `config-services` umbrella |
