...
| Module | Kustomization | Provides |
| --- | --- | --- |
| [ai](https://github.com/ppat/homelab-ops-kubernetes-apps/blob/main/apps/subsystems/ai/README.md) | `apps-ai` | OpenWebUI, LiteLLM AI gateway (master-key auth only, no SSO forward-auth yet) fronting self-hosted mcp-context7, mcp-github, mcp-grafana, mcp-home-assistant, mcp-kubernetes-homelab, mcp-kubernetes-nas, mcp-kubernetes-sandbox, mcp-playwright, mcp-unifi-network, and mcp-unifi-protect MCP servers (the two host-cluster ones are read-only; only the sandbox one is writable — see `services/sandbox-talos/` below), plus n8n (workflow automation) and OpenClaw (WhatsApp gateway that triggers n8n workflows) |
...
| [obsidian-vault](https://github.com/ppat/homelab-ops-kubernetes-apps/blob/main/apps/subsystems/obsidian-vault/README.md) | `apps-obsidian-vault` | A git-backed Obsidian knowledge vault: headless Obsidian holding the only read-write mount on the vault content, two scope-separated MCP servers that are its only network write path, a NATS JetStream broker carrying the durable work queue (reachable off-cluster over TLS for producers outside this cluster), the batch consumer that drains it, a scheduled git committer, and an exporter that independently asserts the vault actually opened. Depends on `apps-ai` for the LiteLLM gateway that fronts the MCP servers and that the batch consumer routes every read and write through |
...
| `services/ci-diagnostics/` | A `CronJob` that pulls the chainsaw suites' published diagnostics grammar (`READY`/`MODE`/`PULL`/`CONTENTION`/`UNCENSORED`…) from the GitHub Actions API into Loki, so the series outlives the 90 days GitHub keeps job logs, with its Grafana dashboard alongside it (`dashboards/ci-diagnostics.json`, discovered by the sidecar's `NAMESPACE=ALL` watch and filed under the `CI` folder). Pulls from the homelab deliberately — no inbound path, no CI-side secret, CI stays ignorant it exists. Its own namespace, because it is the only workload here holding a GitHub credential and reaching the open internet — and therefore its own `config-services-ci-diagnostics` Kustomization, so a first-merge failure (most likely an `ExternalSecret` whose Bitwarden entry does not exist yet) can't fail the shared umbrella. Needs a hand-created Bitwarden entry before it can run; see that directory's `README.md` |
...
| `services/home-automation/` | A second, unreferenced barman-cloud `ObjectStore` (plus its own credential) naming the MinIO copy of `home-automation-db`'s backup history. `components/db-backups` parameterises one object store and mutates it in place, so pointing this database at a different store makes its MinIO history reachable only by reverting that variable and waiting for a reconcile. This keeps the old store addressable by name while the new one is under trial, without disturbing what the trial is measuring. Nothing references it, so it changes no behaviour; note that `components/db-restore` hardcodes `${db_name}-backup-store`, so bootstrapping from this one needs a hand-written `Cluster`. Retire once the object-store migration is done |
...
| `services/longhorn-system/` | Supplies S3 credentials for Longhorn's off-cluster backup targets, plus the `versitygw` `BackupTarget` CR itself. The `default` target is not here — it comes from `storage-core`'s chart via `defaultSettings.backupTarget`; this directory declares the second one, which individual volumes opt into through `Volume.spec.backupTargetName` |
| `services/nas-object-store-trial/` | Two `CronJob`s that write evidence into the one database and the one volume taking part in the MinIO→versitygw object-store trial, so the end-of-trial restores can be checked against something whose survival is a *necessary consequence* of the data having survived rather than against a status field. Hourly: one row carrying `pg_current_wal_lsn()` into a `campaign_sentinel` schema of its own, which supplies the restore's recovery target, the proof that every WAL segment up to it was served back, and the control proving the target bound at all. Nightly at 23:55 UTC, five minutes ahead of the volume's 00:00 backup: a dated sentinel file plus `.campaign-manifest.txt`, a digest of every file in the volume — the only detector for a dropped Longhorn `Blocks[]` entry, which restores a zero-filled hole that nothing store-side can see. Trial-scoped; retire by deleting this directory |
